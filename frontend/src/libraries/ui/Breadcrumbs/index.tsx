...
export function Breadcrumbs({ label }: { label: string }) {
  ...
  return <ul id="breadcrumbs" className="flex flex-wrap gap-2 items-center" aria-label={label}>
    {paths.map((path, indexTODO) =>
      <li className="flex gap-2 items-center h-7.5 not-last:after:bg-[url('/breadcrumb-arrow.svg')] not-last:after:size-4 not-last:after:block" key={path.href === 'fake' ? indexTODO : path.href}>
        <Breadcrumb {...path} />
      </li>,
    )}
  </ul>
